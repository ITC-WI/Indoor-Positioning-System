...
int Trilateration::calculateCoordinates()
{
    ...
    if (mBeaconMeas.size() >= 3)
    {
        for (unsigned int i = 0; i < nOfMeas; i++)
            mostStrongMeas.push_back(mBeaconMeas.at(i));  //Check out again if they really are the strongest or from last of the array is needed.
    }
    ...
    const int UseOls = 0;
    if (UseOls) // How will this go inside then what is the use?
    {
        getLeastSquaresCoordinates();
        getLeastSquaresCoordinates();
    }

#ifdef TRILATERATION_DEBUG_   //Learn the use of if def
...
void Trilateration::filterUnknownBeacons( vector <BeaconMeas>& beaconMeas,
                                          const vector<Beacon>& mapBeacons )   // From where we will generate this mapBeacons?
{
...
int Trilateration::calculateTrilaterationCoordinates()
{
    ...
    vector <double> weight( mBeaconMeas.size(), 0.0 );  //Creating a vector of double with size "mBeaconMeas.size()" and all values initialized with 0.0
